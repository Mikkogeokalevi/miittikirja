...
// ==========================================
// 4. TILASTOJEN JA ELEIDEN HALLINTA
// ==========================================
...
// ==========================================
// 8. TEKSTINTUONTI JA LISTAN LATAUS
// ==========================================
...
function processTextImport(text, mode) {
    ...
    if (lines.length > 0) {
        ...
        if (!ignore.some(p => firstLine.startsWith(p))) { document.getElementById(prefix + 'name').value = firstLine; }
    }
    const dateMatch = text.match(/Tapahtumapäivä:\s*(\d{1,2}\.\d{1,2}\.\d{4})/);
    if (dateMatch) {
        const parts = dateMatch[1].split('.');
        if(parts.length === 3) { document.getElementById(prefix + 'date').value = `${parts[2]}-${parts[1].padStart(2, '0')}-${parts[0].padStart(2, '0')}`; }
    }
...
function loadEvents() {
    ...
    db.ref('miitit/' + currentUser.uid + '/events').on('value', (snapshot) => {
        Object.values(lists).forEach(l => { if(l.past) l.past.innerHTML = ""; if(l.future) l.future.innerHTML = ""; });
        const events = [];
        ...
        events.forEach(evt => {
            ...
            db.ref('miitit/' + currentUser.uid + '/logs/' + evt.key).once('value').then((snap) => {
                const el = document.getElementById(countId); if (el) el.innerText = "👤 " + snap.numChildren();
            });
...
window.openGuestbook = (eventKey) => {
    ...
    db.ref('miitit/' + currentUser.uid + '/events/' + eventKey).on('value', snap => {
        ...
        if (descEl) {
            if (evt.descriptionHtml) { descEl.innerHTML = evt.descriptionHtml; document.getElementById('box-desc').style.display = 'block'; }
...
document.getElementById('btn-save-edit').onclick = () => {
    db.ref('miitit/' + currentUser.uid + '/events/' + document.getElementById('edit-key').value).update({
    ...
    }).then(() => { if(editModal) editModal.style.display = "none"; });
};

window.deleteEvent = async (k) => { 
    const confirmed = await customConfirm("Poista miitti", "Haluatko varmasti poistaa tämän miitin ja kaikki sen kirjaukset? Tätä ei voi perua.");
    if (confirmed) { 
...
window.deleteLog = async (lk) => { 
    const confirmed = await customConfirm("Poista kirjaus", "Haluatko varmasti poistaa tämän kävijän?");
...
document.getElementById('btn-save-log-edit').onclick = () => {
    ...
    }).then(() => { if(logEditModal) logEditModal.style.display = "none"; });
};
...
document.getElementById('btn-logout').onclick = async () => { 
    const confirmed = await customConfirm("Kirjaudu ulos", "Haluatko varmasti kirjautua ulos?");
