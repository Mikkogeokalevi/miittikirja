...
// ==========================================
// 8. TAPAHTUMIEN LATAUS JA "TÄNÄÄN TAPAHTUU"
// ==========================================
...
document.getElementById('btn-visitor-sign').onclick = function() {
    ...
    db.ref('miitit/' + mikkokaleviUid + '/logs/' + currentEventId).push({
        nickname: nick, 
        ...
        location.href = "https://www.geocaching.com"; 
    });
};
...
function loadAttendees(eventKey) {
    const uid = currentUser ? currentUser.uid : "T8wI16Gf67W4G4yX3Cq7U0U1H6I2";
    db.ref('miitit/' + uid + '/logs/' + eventKey).on('value', (snapshot) => {
