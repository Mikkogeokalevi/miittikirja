...
const visitorTranslations = {
    ...
    en: {
        ...
        badgeDescs: { first: "Sign your first meet", ten: "Visit 10 meets", twentyFive: "Visit 25 meets", fifty: "Visit 50 meets", seventyFive: "Visit 75 meets", hundred: "Visit 100 meets", hundredTwentyFive: "Visit 125 meets", hundredFifty: "Visit 150 meets", twoHundred: "Visit 200 meets", rankSeikkailija: "Earned at 2 visits", rankAktiivi: "Earned at 11 visits", rankVakio: "Earned at 21 visits", rankKonkari: "Earned at 41 visits", vip: "Earned at 61 visits", streak5: "5 meets in a row", streak10: "10 meets in a row", streak20: "20 meets in a row", streak30: "30 meets in a row", streak40: "40 meets in a row", wordMaster: "500+ words in messages total", poet: "One message with 50+ words", chatter: "20+ messages written", netLogger: "Message from geocaching.com too", earlyBird: "Signed between 05–08", morning: "Signed between 08–11", lunch: "Signed between 11–14", afternoon: "Signed between 14–17", evening: "Signed between 17–20", lateEvening: "Signed between 20–22", nightOwl: "Signed between 22–00", midnight: "Signed between 00–05", fullClock: "Signed in all 8 time slots", podium: "Visitor rank 1–3", topTen: "Visitor rank 4–10", buddy: "10+ shared meets with the same visitor", traveler: "Logs from 3+ different places", veteran: "Visits in 3+ different years", yearRound: "Visited in all 12 months", citoVisitor: "Attend a CITO event", cceVisitor: "Attend a community celebration", allRounder: "Visit all 3 event types", milestone: "Exactly 10/20/30... visits" },
...
// Näyttää badgen selitteen info-laatikossa (klikkaus mobiililla / hover desktopilla)
window.showBadgeInfo = function(el) {
    const key = el.getAttribute('data-badge-key');
    const t = visitorTranslations[currentLang] || visitorTranslations.fi;
    const desc = (t.badgeDescs && t.badgeDescs[key]) || '';
    const name = (t.badgeNames && t.badgeNames[key]) || key;
    const infoBox = document.getElementById('visitor-badge-info');
    if (!infoBox) return;
    if (desc) {
        infoBox.innerHTML = '<strong>' + name + '</strong> — ' + desc;
...
function showVisitorModalWithLang(nick, history, stats) {
    ...
    // Badge-kokoelma: ansaitut + puuttuvat (klikattavat — näyttävät selitteen)
    const badgeNames = t.badgeNames || {};
    const badgeDescs = t.badgeDescs || {};
    ...
    const badgesHtml = (earnedCount > 0 || missingBadges.length > 0)
        ? `<div class="visitor-summary-title" style="margin-top:12px;">${t.badgeCollectionTitle || '🏅 Saavutukset'} ${earnedCount}/${badgeTotal}</div>
           ...
           ${missingBadges.length > 0 ? `<div class="visitor-badge-missing-title">${t.badgeMissingTitle || 'Vielä saamatta'}:</div><div class="visitor-badge-collection">${missingBadges.map(b => badgeChip(b, true)).join('')}</div>` : ''}
