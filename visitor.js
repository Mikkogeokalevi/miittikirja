...
// Miitin alkamistunti time-kentästä (esim. "20:15-20:45" -> 20), null jos ei aikaa
function getEventStartHour(timeStr) {
...
async function computeVisitorStats(targetHost, eventId, nick, virtualLog, preloaded) {
    ...
    try {
        ...
        const eventFirstSigner = {}; // eventKey -> { nick, ts } (aikaisin kirjautuja)
        ...
        logsSnap.forEach(evtLogs => {
            ...
            if (evtData) {
                ...
                evtLogs.forEach(log => {
                    ...
                    if (val.timestamp) {
                        const prev = eventFirstSigner[eventKey];
                        if (!prev || val.timestamp < prev.ts) {
                            eventFirstSigner[eventKey] = { nick: ln, ts: val.timestamp };
                        }
                    }

                    if (ln === nickNorm) {
                        attended = true;
                        // Aikabadget miitin alkamisajan mukaan (fallback: kirjaushetki)
                        const evtHour = getEventStartHour(evtData.time);
                        if (evtHour !== null) {
                            userLogTimeBuckets.add(getLogTimeBucket(evtHour));
            ...
            if (evtData) {
                ...
                if (ln) {
                    ...
                    if (ln === nickNorm) {
                        ...
                        const vEvtHour = getEventStartHour(evtData.time);
                        if (vEvtHour !== null) {
                            userLogTimeBuckets.add(getLogTimeBucket(vEvtHour));
                        } else if (virtualLog.timestamp) {
        ...
        // --- Erityisbadgejen laskenta ---
        // Tuplakävijä: 2+ tapahtumaa samana päivänä
        const dayCounts = {};
        userHistory.forEach(e => {
            const d = (e.date || '').slice(0, 10);
            if (d) dayCounts[d] = (dayCounts[d] || 0) + 1;
        });
        const hasDoubleDay = Object.values(dayCounts).some(c => c >= 2);

        // Massamiittari: osallistunut 20+ vieraan tapahtumaan
        const hasBigMeet = userHistory.some(e => (eventAttendees[e.key] || new Set()).size >= 20);

        // Neljän vuodenajan vieras: käynti joka vuodenaikana (0=talvi,1=kevät,2=kesä,3=syksy)
        const seasonSet = new Set(userHistory.map(e => {
            const m = parseInt((e.date || '').slice(5, 7), 10);
            return m >= 1 && m <= 12 ? Math.floor((m % 12) / 3) : null;
        }).filter(s => s !== null));
        const hasAllSeasons = seasonSet.size >= 4;

        // Avausvieras: ensimmäinen kirjautuja jossain miitissä
        const hasFirstSigning = Object.values(eventFirstSigner).some(f => f.nick === nickNorm);
        ...
        const badgeCatalog = [
            ...
            { icon: '💫', key: 'rankSuperVip', earned: stats.totalVisits >= 80 },
            { icon: '🌠', key: 'rankLegenda', earned: stats.totalVisits >= 100 },
            { icon: '🔱', key: 'rankElavaLegenda', earned: stats.totalVisits >= 125 },
            // Putket
            ...
            { icon: '💌', key: 'firstMessage', earned: messageCount >= 1 },
            { icon: '🖋️', key: 'writer', earned: messageCount >= 50 },
            { icon: '📚', key: 'author', earned: stats.messageWordTotal >= 1000 },
            { icon: '🗣️', key: 'alwaysMessage', earned: stats.totalVisits >= 10 && messageCount >= stats.totalVisits },
            { icon: '📈', key: 'regularWriter', earned: stats.totalVisits >= 10 && (messageCount / stats.totalVisits) >= 0.5 },
            ...
            { icon: '🏅', key: 'topTen', earned: stats.rankPosition > 0 && stats.rankPosition <= 10 },
            { icon: '🤝', key: 'buddy', earned: stats.topBuddies.length > 0 && stats.topBuddies[0].count >= 10 },
            { icon: '📍', key: 'hometown', earned: fromEntries.length >= 1 },
            { icon: '🌍', key: 'traveler', earned: fromEntries.length >= 3 },
            { icon: '🧭', key: 'explorer', earned: fromEntries.length >= 5 },
            { icon: '🌏', key: 'globeTrotter', earned: fromEntries.length >= 10 },
            ...
            // Erityiset
            { icon: '🎰', key: 'doubleDay', earned: hasDoubleDay },
            { icon: '🥇', key: 'firstSigner', earned: hasFirstSigning },
